...
import { OperatoTerms } from '@things-factory/operatofill'
import { PageView } from '@things-factory/shell'

class SystemHome extends localize(i18next)(PageView) {
  static get styles() {
    return [
      css `
        :host {
  ...
  render() {
    ...
    return html `
      <h2>System</h2>
      <p page-description>
        이 메뉴는 Operato Logistics 시스템 정보를 수정/관리 하는 메뉴입니다.
      </p>
      
      <img src="/assets/images/system/system-home.png" />      
    `
  }
  ...
  async pageUpdated(changes, lifecycle, before) {
    if (this.active) {
       ...
       */
      console.log('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>')
      console.log(OperatoTerms.t1('label.name'))
      console.log(OperatoTerms.t2('label', 'name'))
      console.log(OperatoTerms.t3('error.SHOULD_NOT_BE_EMPTY', { value: '입력 ...' }))
      console.log(OperatoTerms.t4('text', 'Elapsed Seconds', { days: '1', hours: '2', minutes: '3', seconds: '4' }))
      console.log('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>')
    } else {
...
window.customElements.define('system-home', SystemHome)