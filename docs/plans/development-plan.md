...
| 구분 | 기술 | 버전 | 비고 |
|------|------|------|------|
| 백엔드 프레임워크 | Spring Boot | 3.2.4 | Java 17+ 필요 |
| 빌드 도구 | Gradle | 최신 안정 버전 | Groovy DSL 사용 |
| 프론트엔드 프레임워크 | Things Factory | - | Lit 기반 Web Components |
| 프론트엔드 구조 | Lerna 모노레포 | - | 4개 패키지 (wcs-ui, system-ui, metapage, operatofill) |
| UI 컴포넌트 | Lit | 3.x | Web Components 표준 기반 |
...
- **모노레포 프론트엔드**: Things Factory 기반 Lerna 모노레포, Lit 컴포넌트 단위 독립 개발
...
**백엔드**
- [x] Spring Boot 3.2.4 프로젝트 초기화
- [x] Gradle 멀티 모듈 구조 설정 (`otarepo-core` 분리 완료)
- [x] 도메인 모듈 패키지 분리 완료
  - [x] `operato.logis.*` — ECS 모듈별 도메인 (DAS, DPS, SPS, SMS 등)
  - [x] `xyz.anythings.*` — WCS 공통 도메인 (Base, GW, Sys, Sec)
  - [x] `xyz.elidom.*` — 프레임워크 코어
- [ ] (선택) Gradle 멀티 모듈로 물리적 분리 (`wcs-api`, `wcs-domain` 등)

**프론트엔드**
- [x] Things Factory 기반 Lerna 모노레포 구성 (완료: 2026-03-11)
  - [x] `operato-wcs-ui` — 메인 WCS UI (포트 5908)
  - [x] `operato-wcs-system-ui` — 시스템 UI
  - [x] `metapage` — 메타페이지
  - [x] `operatofill` — Spring 백엔드 연동
- [x] 공통 컴포넌트 라이브러리 구조 설정 (Things Factory 프레임워크 내장)
- [x] 라우터 설정 (Things Factory 프레임워크 내장)
- [ ] (선택) 디자인 시스템 정의 (CSS Custom Properties 커스터마이징)

**인프라**
- [x] Docker Compose 환경 구성 (PostgreSQL, RabbitMQ, Redis)
- [ ] CI/CD 파이프라인 초기 구성 (GitHub Actions)
- [x] 개발 환경 문서화 (CLAUDE.md, deployment.md, architecture 문서, 코딩 컨벤션 등)
...
Manager, Kiosk, PDA, Tablet, Monitoring Board 등 다양한 클라이언트 UI를 Things Factory (Lit 기반 Web Components)로 개발합니다.
...
| 프론트엔드 개발자 | 2명 | Things Factory, Lit 컴포넌트, PWA |
...
| 2026-03-11 | 1.0.1 | 프론트엔드 Things Factory 모노레포 반영, Spring Boot 버전 현행화 (3.2.4) | Claude Code |
