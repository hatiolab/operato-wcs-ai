...
|----|------|----------|----------|----------|----------|--------|------|
| 1.1.1 | BE | 백엔드 초기화 | Spring Boot 3.2.4 프로젝트 생성 및 기본 설정 | 2026-03-01 | ✅ | 100% | build.gradle, settings.gradle 완료 |
| 1.1.2 | BE | 백엔드 구조 | Gradle 멀티 모듈 구조 설정 (otarepo-core 분리) | 2026-03-01 | ✅ | 100% | 패키지 수준 도메인 분리 완료. 물리적 모듈 분리는 선택사항 |
| 1.1.3 | BE | 공통 라이브러리 | 공통 유틸리티, 예외 처리, 응답 포맷 구현 | 2026-03-01 | ✅ | 100% | otarepo-core 모듈로 분리 완료 |
| 1.1.4 | FE | 프론트엔드 초기화 | Things Factory 기반 Lerna 모노레포 구성 | 2026-03-11 | ✅ | 100% | frontend/ 디렉토리, 4개 패키지 구성 완료 |
| 1.1.5 | FE | 디자인 시스템 | CSS Custom Properties 정의 (색상, 폰트, 간격 등) | - | ⚪ | 0% | 선택사항. Things Factory 기본 스타일 사용 가능 |
| 1.1.6 | FE | 컴포넌트 구조 | 공통 컴포넌트 라이브러리 구조 설정 | 2026-03-11 | ✅ | 100% | Things Factory 프레임워크 내장 |
| 1.1.7 | FE | 라우터 설정 | 클라이언트 사이드 라우터 설정 | 2026-03-11 | ✅ | 100% | Things Factory 프레임워크 내장 |
| 1.1.8 | DO | 인프라 구성 | Docker Compose 환경 구성 (PostgreSQL, RabbitMQ, Redis) | 2026-03-05 | ✅ | 100% | docker-compose.yml, docker-compose.prod.yml |
| 1.1.9 | DO | CI/CD 초기 구성 | GitHub Actions 워크플로우 생성 (빌드, 테스트) | - | ⚪ | 0% | .github/workflows/ 디렉토리 미생성 |
| 1.1.10 | PM | 개발 환경 문서화 | 개발 환경 설정 가이드 작성 | 2026-03-11 | ✅ | 100% | CLAUDE.md, deployment.md, architecture 문서 등 완료 |
...
> **Note**: Things Factory 프레임워크에 디자인 시스템 및 기본 컴포넌트가 내장되어 있으므로, 별도 구축 불필요

### 3.1 Manager UI (Week 21-23)

| ID | 역할 | 작업 구분 | 작업 상세 | 완료 일정 | 완료 여부 | 진행률 | 비고 |
|----|------|----------|----------|----------|----------|--------|------|
| 3.1.1 | FE | 대시보드 레이아웃 | 대시보드 레이아웃 및 위젯 시스템 | Week 21-Day 1 | ⚪ | 0% | 그리드 레이아웃 |
| 3.1.2 | FE | 주문 현황 위젯 | 주문 현황 요약 위젯 | Week 21-Day 2 | ⚪ | 0% | 금일 입고/출고/완료 |
| 3.1.3 | FE | 설비 현황 위젯 | 설비 상태 요약 위젯 | Week 21-Day 3 | ⚪ | 0% | 가동/점검/오류 |
| 3.1.4 | FE | 작업자 현황 위젯 | 작업자 현황 위젯 | Week 21-Day 4 | ⚪ | 0% | 출근/작업 중/휴게 |
| 3.1.5 | FE | 실시간 알림 | 실시간 알림 시스템 (WebSocket) | Week 21-Day 5 | ⚪ | 0% | 오류, 경고, 완료 |
| 3.1.6 | FE | 주문 목록 | 주문 목록 화면 (필터, 정렬, 검색) | Week 22-Day 1 | ⚪ | 0% | DataGrid 활용 |
| 3.1.7 | FE | 주문 상세 | 주문 상세 조회 및 분배 현황 | Week 22-Day 2 | ⚪ | 0% | 타임라인 UI |
| 3.1.8 | FE | 주문 수동 분배 | 주문 수동 분배 화면 | Week 22-Day 3 | ⚪ | 0% | 설비 선택 UI |
| 3.1.9 | FE | 설비 목록 | 설비 목록 및 상태 관리 화면 | Week 22-Day 4 | ⚪ | 0% | 카드 레이아웃 |
| 3.1.10 | FE | 설비 모니터링 | 설비 실시간 모니터링 화면 | Week 22-Day 5 | ⚪ | 0% | WebSocket 연동 |
| 3.1.11 | FE | 마스터 관리 | 상품·작업자·설비 관리 통합 화면 | Week 23-Day 1-2 | ⚪ | 0% | CRUD 통합 |
| 3.1.12 | FE | 실적 조회 | 실적 조회 화면 (일별, 월별) | Week 23-Day 3 | ⚪ | 0% | 날짜 필터 |
| 3.1.13 | FE | 실적 차트 | 실적 대시보드 (차트, 그래프) | Week 23-Day 4 | ⚪ | 0% | Chart.js |
| 3.1.14 | FE | 보고서 다운로드 | 실적 보고서 다운로드 (Excel, PDF) | Week 23-Day 5 | ⚪ | 0% | API 연동 |

### 3.2 Kiosk UI (Week 24)

| ID | 역할 | 작업 구분 | 작업 상세 | 완료 일정 | 완료 여부 | 진행률 | 비고 |
|----|------|----------|----------|----------|----------|--------|------|
| 3.2.1 | FE | Kiosk 로그인 | 작업자 로그인 화면 (바코드 스캔) | Week 24-Day 1 | ⚪ | 0% | 카메라 연동 |
| 3.2.2 | FE | 출퇴근 처리 | 출근·퇴근 처리 화면 | Week 24-Day 2 | ⚪ | 0% | 시간 기록 |
| 3.2.3 | FE | 작업 지시 확인 | 작업 지시 확인 화면 | Week 24-Day 3 | ⚪ | 0% | 할당된 작업 |
| 3.2.4 | FE | 휴게 관리 | 휴게 시간 관리 화면 | Week 24-Day 4 | ⚪ | 0% | 타이머 |
| 3.2.5 | QA | Kiosk UI 테스트 | Kiosk 화면 통합 테스트 | Week 24-Day 5 | ⚪ | 0% | 터치 스크린 |

### 3.3 PDA UI (Week 25-26)

| ID | 역할 | 작업 구분 | 작업 상세 | 완료 일정 | 완료 여부 | 진행률 | 비고 |
|----|------|----------|----------|----------|----------|--------|------|
| 3.3.1 | FE | PDA 공통 레이아웃 | PDA 공통 레이아웃 및 네비게이션 | Week 25-Day 1 | ⚪ | 0% | 모바일 최적화 |
| 3.3.2 | FE | 입고 PDA UI | 입고 PDA 화면 (앞서 구현한 것 개선) | Week 25-Day 2 | ⚪ | 0% | UX 개선 |
| 3.3.3 | FE | P-DAS PDA UI | P-DAS PDA 화면 (앞서 구현한 것 개선) | Week 25-Day 3 | ⚪ | 0% | UX 개선 |
| 3.3.4 | FE | 작업 이력 | 작업 이력 조회 화면 | Week 25-Day 4 | ⚪ | 0% | 무한 스크롤 |
| 3.3.5 | FE | PDA 설정 | PDA 설정 화면 (언어, 알림 등) | Week 25-Day 5 | ⚪ | 0% | Local Storage |
| 3.3.6 | FE | 오프라인 모드 | 오프라인 모드 지원 (PWA) | Week 26-Day 1-2 | ⚪ | 0% | Service Worker |
| 3.3.7 | FE | 푸시 알림 | 푸시 알림 기능 구현 | Week 26-Day 3 | ⚪ | 0% | Web Push API |
| 3.3.8 | QA | PDA UI 테스트 | PDA 화면 통합 테스트 | Week 26-Day 4-5 | ⚪ | 0% | 모바일 단말 |

### 3.4 Monitoring Board UI (Week 27-28)

| ID | 역할 | 작업 구분 | 작업 상세 | 완료 일정 | 완료 여부 | 진행률 | 비고 |
|----|------|----------|----------|----------|----------|--------|------|
| 3.4.1 | FE | 모니터링 레이아웃 | 대형 화면용 레이아웃 (Full Screen) | Week 27-Day 1 | ⚪ | 0% | TV/모니터 최적화 |
| 3.4.2 | FE | 설비 상태 맵 | 설비 상태 맵 (전체 설비 한눈에) | Week 27-Day 2 | ⚪ | 0% | SVG 또는 Canvas |
| 3.4.3 | FE | 주문 진행 현황 | 주문 진행 현황 (파이 차트, 바 차트) | Week 27-Day 3 | ⚪ | 0% | 애니메이션 차트 |
| 3.4.4 | FE | 작업자 현황 | 작업자 현황 (출근 인원, 작업 중) | Week 27-Day 4 | ⚪ | 0% | 실시간 갱신 |
| 3.4.5 | FE | 실시간 알림 | 실시간 알림 표시 (오류, 경고) | Week 27-Day 5 | ⚪ | 0% | 팝업 알림 |
| 3.4.6 | FE | WebSocket 연동 | WebSocket 기반 실시간 데이터 갱신 | Week 28-Day 1 | ⚪ | 0% | 자동 재연결 |
| 3.4.7 | FE | 3D 재고 모니터링 | 3D 재고 모니터링 (선택 사항) | Week 28-Day 2-3 | ⚪ | 0% | Three.js |
| 3.4.8 | FE | 자동 갱신 | 데이터 자동 갱신 및 캐싱 | Week 28-Day 4 | ⚪ | 0% | 5초 간격 |
| 3.4.9 | QA | 모니터링 UI 테스트 | 모니터링 보드 통합 테스트 | Week 28-Day 5 | ⚪ | 0% | 대형 화면 |
...
| P3-C5 | Things Factory 컴포넌트 재사용성 80% 이상 | FE | ⚪ |
