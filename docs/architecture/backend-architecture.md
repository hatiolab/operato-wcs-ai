...
```
src/main/java/
├── xyz.anythings.*        # WCS 비즈니스 도메인 공통 모듈 (347 파일)
├── xyz.elidom.*           # 공통 프레임워크 모듈 (634 파일)
├── operato.logis.*        # ECS 모듈별 도메인 구현 (216 파일)
├── operato.core.*         # Operato 제품군 공통 모듈 (2 파일)
├── net.sf.*               # 외부 유틸리티
├── org.*                  # 확장/수정된 외부 라이브러리 (2 파일)
└── report.*               # 리포트 생성 관련 모듈
```
...
### 3.4 operato.logis 패키지 (ECS 모듈별 도메인)

**ECS(Equipment Control System) 모듈별 비즈니스 도메인**을 담당하는 패키지입니다. 각 모듈은 특정 설비 유형에 대한 제어 로직을 캡슐화합니다.

총 **10개 ECS 모듈**, **191개 Java 파일**로 구성되어 있습니다.

#### 3.4.1 모듈 목록

| 패키지 | 설명 | 파일 수 | 주요 기능 |
|--------|------|---------|----------|
| **operato.logis.bms** | BMS (Batch Management System) | 10 | - 배치 관리 시스템<br>- 배치 작업 제어<br>- 작업 이력 관리 |
| **operato.logis.das** | DAS (Digital Assorting System) | 33 | - 디지털 분류 시스템<br>- DAS 작업 관리<br>- 셀 매핑 및 투입 제어 |
| **operato.logis.dpc** | DPC (Digital Product Coding) | 23 | - 디지털 상품 코딩<br>- 제품 식별 및 추적<br>- 바코드/RFID 관리 |
| **operato.logis.dps** | DPS (Digital Picking System) | 28 | - 디지털 피킹 시스템<br>- DPS 작업 관리<br>- 피킹 리스트 생성 |
| **operato.logis.insp** | Inspection (검수) | 7 | - 출고 검수 시스템<br>- 품질 검사<br>- 검수 이력 관리 |
| **operato.logis.ops** | OPS (Order Picking System) | 7 | - 주문 피킹 시스템<br>- 피킹 최적화<br>- 작업 할당 |
| **operato.logis.pdas** | PDAS (Pick-to-Light DAS) | 23 | - 피킹 표시등 DAS<br>- 표시기 제어<br>- 피킹 가이드 |
| **operato.logis.sms** | SMS (Sorter Management System) | 23 | - 소터 관리 시스템<br>- 대량 분류 작업<br>- No-Read 처리 |
| **operato.logis.sps** | SPS (Single Packing System) | 18 | - 단품 포장 시스템<br>- 포장 작업 관리<br>- 슈트 매핑 |
| **operato.logis.wcs** | WCS (Warehouse Control System) | 19 | - WCS 핵심 로직<br>- 설비 통합 제어<br>- 작업 스케줄링 |

#### 3.4.2 패키지 네이밍 규칙

```
operato.logis.{모듈명}.{계층}
```

예시:
- `operato.logis.das.config` — DAS 모듈 설정
- `operato.logis.dps.service` — DPS 모듈 서비스
- `operato.logis.sms.query.postgresql` — SMS 모듈 PostgreSQL 쿼리

#### 3.4.3 공통 모듈 구조

모든 `operato.logis.*` 모듈은 일관된 레이어 구조를 따릅니다:

```
operato.logis.{module}/
├── config/             # 모듈 설정 (ModuleProperties, Bean 정의)
├── model/              # DTO, VO (데이터 전송 객체) *
├── entity/             # JPA 엔티티 (도메인 모델) *
├── service/            # 비즈니스 서비스 로직
├── web/                # REST 컨트롤러 (웹 API)
├── rest/               # REST 컨트롤러 (일부 모듈만 사용) *
├── query/              # 데이터베이스별 쿼리 (복잡한 쿼리 관리) *
│   ├── postgresql/     # PostgreSQL 전용 쿼리
│   ├── oracle/         # Oracle 전용 쿼리
│   ├── mysql/          # MySQL 전용 쿼리
│   └── mssql/          # MS SQL Server 전용 쿼리
├── event/              # 이벤트 정의 (일부 모듈만 사용) *
├── job/                # 배치 작업 (일부 모듈만 사용) *
└── util/               # 유틸리티 (일부 모듈만 사용) *

* 선택적 패키지 (모듈에 따라 존재 여부 다름)
```

#### 3.4.4 모듈별 패키지 구성 상세

| 모듈 | config | model | entity | service | web | rest | query | event | job | util |
|------|--------|-------|--------|---------|-----|------|-------|-------|-----|------|
| bms  | ✓ | ✓ | - | ✓ | ✓ | ✓ | - | - | - | ✓ |
| das  | ✓ | - | - | ✓ | ✓ | - | ✓ | - | - | - |
| dpc  | ✓ | ✓ | - | ✓ | ✓ | - | ✓ | - | - | - |
...
**주요 특징**:
- 모든 모듈이 `config/`, `service/`, `web/` 패키지를 공통으로 보유
- 대부분의 모듈이 `query/` 패키지를 통해 다중 데이터베이스(PostgreSQL, Oracle, MySQL, MSSQL) 지원
- `wcs` 모듈이 가장 복잡한 구조를 가지며 `event/`, `job/` 패키지를 포함
- `entity/`는 일부 모듈(sms, wcs)만 보유, 대부분은 `model/` 패키지 사용

#### 3.4.5 모듈 설정 패턴

각 모듈은 `ModuleProperties` 클래스를 통해 설정을 관리합니다:

```java
@Component("operatoLogis{Module}ModuleProperties")
@PropertySource("classpath:/properties/operato-logis-{module}.properties")
public class ModuleProperties implements IModuleProperties {
    @Value("${operato.logis.{module}.name}") private String name;
    @Value("${operato.logis.{module}.description}") private String description;
    @Value("${operato.logis.{module}.version}") private String version;
    @Value("${operato.logis.{module}.basePackage}") private String basePackage;
    // ...
}
```

모듈별 설정 파일 위치: `src/main/resources/properties/operato-logis-{module}.properties`

### 3.5 operato.core 패키지
...
| 1.1 | 2026-03-11 | operato.logis 패키지 상세 분석 및 섹션 3.4 업데이트 — 10개 ECS 모듈 목록, 패키지 구조, 모듈별 계층 매트릭스 추가 | Claude Code |
