 ...
 */
package xyz.elidom.dbist.dml.impl;
...
import xyz.elidom.dbist.dml.AbstractDml;
...
import xyz.elidom.dbist.dml.jdbc.QueryMapperCassandra;
import xyz.elidom.dbist.dml.jdbc.QueryMapperDb2;
import xyz.elidom.dbist.dml.jdbc.QueryMapperH2;
import xyz.elidom.dbist.dml.jdbc.QueryMapperMysql;
import xyz.elidom.dbist.dml.jdbc.QueryMapperOracle;
import xyz.elidom.dbist.dml.jdbc.QueryMapperPostgresql;
import xyz.elidom.dbist.dml.jdbc.QueryMapperSqlserver;
import xyz.elidom.dbist.exception.DataNotFoundException;
import xyz.elidom.dbist.exception.DbistRuntimeException;
...
public class DmlJdbc2 extends AbstractDml implements Dml {
	...
	private static void setFieldValue(ResultSet rs, int index, Object data, Field field, Field subField) throws SQLException {
		...
		if (subData == null) {
			try {
				// subData = field.getType().getClass().getConstructor().newInstance();
	...
	public List<TableColumn> getTableColumnList(Table table) {
		...
		return jdbcOperations.query(sql, this.getTableColumnRowMapper(), new Object[] { tableName });
	}
	...
	private void addColumn(Table table, Field field) {
		...
		// Column
		{
			...
			sql = StringUtils.replace(sql, "${domain}", toFirstDomainName(table.getDomain()));
			xyz.elidom.dbist.annotation.Column columnAnn = field.getAnnotation(xyz.elidom.dbist.annotation.Column.class);
		...
		// Identity / Sequence
		xyz.elidom.dbist.annotation.Sequence seqAnn = field.getAnnotation(xyz.elidom.dbist.annotation.Sequence.class);
		if (seqAnn != null) {
