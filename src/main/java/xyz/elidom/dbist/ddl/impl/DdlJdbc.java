package xyz.elidom.dbist.ddl.impl;
...
import xyz.elidom.dbist.ddl.AbstractDdl;
...
public class DdlJdbc extends AbstractDdl {
	...
	public void setTableSpace(String dataTBSpace,String idxTBSpace) {
		...
		if(ValueUtil.isEmpty(dataTBSpace)) this.useDataTBSpace = false;
		else {
			this.useDataTBSpace = true;
			this.dataTBSpace = dataTBSpace;
		}
		
		if(ValueUtil.isEmpty(idxTBSpace)) this.useIdxTBSpace = false;
		else {
	...
	public String createTable(Class<?> entity) {
		...
		String pkFields = primaryFields.toString();
		paramMap.put("primaryKeys", pkFields);
		paramMap.put("pkExist", ValueUtil.isNotEmpty(pkFields));
		...
		// Template 가져오기
		String template = this.getDdlMapper().getDdlTemplate();
		// tableName, template, paramMap을 각 데이터베이스 DdlMapper에서 핸들링 (For Customizing)
		this.getDdlMapper().handleTableCreationParameters(tableName, template, paramMap);
