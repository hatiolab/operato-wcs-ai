...
import xyz.elidom.dbist.ddl.mapper.DdlMapperDB2;
import xyz.elidom.dbist.ddl.mapper.DdlMapperH2;
import xyz.elidom.dbist.ddl.mapper.DdlMapperMysql;
import xyz.elidom.dbist.ddl.mapper.DdlMapperOracle;
import xyz.elidom.dbist.ddl.mapper.DdlMapperPostgresql;
import xyz.elidom.dbist.ddl.mapper.DdlMapperSqlserver;
...
public abstract class AbstractDdl implements Ddl {
	...
	static class TableColumnRowMapper implements RowMapper<TableCol> {
		public TableCol mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			tabColumn.setLength(rs.getLong("length"));
	...
	@Override
	public xyz.elidom.dbist.metadata.Table getTable(String tableName) {
		return this.dml.getTable(tableName);
	}

	@Override
	public xyz.elidom.dbist.metadata.Table getTable(Class<?> entityClass) {
	...
	public List<TableIdx> getTableIndexes(Class<?> entityClass) {
		...
		xyz.elidom.dbist.annotation.Index[] indexes = (xyz.elidom.dbist.annotation.Index[]) tableInfo.get("indexes");
		String tableName = (String) tableInfo.get("name");

		for (xyz.elidom.dbist.annotation.Index idx : indexes) {
	...
	@Override
	public List<TableCol> getTableCols(xyz.elidom.dbist.metadata.Table table) {
		...
		return this.dml.getJdbcOperations().query(sql, TABLECOLUMN_ROWMAPPER, new Object[] { table.getName() } );
	}
