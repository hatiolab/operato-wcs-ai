package xyz.elidom.dbist.ddl.mapper;
 ...
 * @author shortstop
 */
public class DdlMapperMysql extends DdlMapperAbstract implements DdlMapper {
	...
	public String getDdlTemplate() {
		...
		joiner.add("		, UNIQUE INDEX $index.name ($index.columnList) USING BTREE");
		joiner.add("	#else");
		joiner.add("		, INDEX $index.name ($index.columnList) USING BTREE");
		joiner.add("	#end");
		joiner.add("\n");
		joiner.add("#end");

		// Create Index Template
		template.append(joiner.toString());
	...
	public String tableTemplate() {
		...
		joiner.add("#else");
		// 예약어 (ex: rank) 컬럼명인 경우는 에러가 발생할 수 있으니 컬럼명 앞 뒤에 ` 문자를 붙인다.
	...
	public String getTableCheckSql() {
		...
		return "select 1 from information_schema.tables where lower(table_schema) = '${domain}' and lower(table_name) = '$tableName'";
	}
	...
	public String removeColumnTemplate() {
		...
		joiner.add("	ALTER TABLE $tableName DROP COLUMN `$column.name`;\n");
		joiner.add("#end");
		return joiner.toString();
	}
	...
	public String modifyColumnTemplate() {
		...
		joiner.add("	ALTER TABLE $tableName MODIFY COLUMN `$column.name` $column.type;\n");
		joiner.add("#end");
		return joiner.toString();
	}
	...
	public String renameColumnTemplate() {
		...
		joiner.add("	ALTER TABLE $tableName CHANGE COLUMN `$column.nameFrom` `$column.nameTo $column.type`;\n");
		joiner.add("#end");
		return joiner.toString();
	}
	...
	public String modifyNullableColumnTemplate() {
		...
		joiner.add("	#end");
		joiner.add("	#if($column.nullable && $column.nullable == 'NO')");
		joiner.add("		ALTER TABLE $tableName MODIFY COLUMN `$column.name` $column.type NOT NULL;\n");
		joiner.add("	#end");
		joiner.add("#end");
		return joiner.toString();
	}
	...
	@Override
	public void handleTableCreationParameters(String tableName, String template, Map<String, Object> paramMap) {
		@SuppressWarnings("unchecked")
		List<Map<String, Object>> indexList = (List<Map<String, Object>>)paramMap.get("indexes");
		for(Map<String, Object> indexObj : indexList) {
			String columns = (String)indexObj.get("columnList");
			String[] columnArr = columns.split(SysConstants.COMMA);
			StringJoiner newColumns = new StringJoiner(SysConstants.COMMA);
			for(int i = 0 ; i < columnArr.length ; i++) {
				newColumns.add("`" + columnArr[i].trim() + "`");
			}
			indexObj.put("columnList", newColumns.toString());
		}
	}
