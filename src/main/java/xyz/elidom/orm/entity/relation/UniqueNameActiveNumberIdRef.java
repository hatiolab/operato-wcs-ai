 ...
 * @author shortstop
 */
public abstract class UniqueNameActiveNumberIdRef implements IUniqueNameRef {
	...
	public Object findAndSetId() {
		...
		if(ValueUtil.isNotEmpty(domainId) && ValueUtil.isNotEmpty(this.getName()) && ValueUtil.isNotEmpty(this.getVersion())) {
			...
			String sql = "select id from " + tableInfoMap.get("name") + " where domain_id = :domainId and name = :name and version = :version";
			IQueryManager queryMan = BeanUtil.get(IQueryManager.class);
			Map<String, Object> params = ValueUtil.newMap("domainId,name,version", Domain.currentDomain().getId(), this.getName(), this.getVersion()); 
