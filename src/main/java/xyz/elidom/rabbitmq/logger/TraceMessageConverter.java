...
public class TraceMessageConverter {
	...
	public static ITraceModel convertPublishMessage(org.springframework.amqp.core.MessageProperties messageProperties, byte[] body) throws Exception{
		...
		String[] routingKeyArr = ValueUtil.isEmpty(routingKeyStr) ? new String[0] : routingKeyStr.split(",");
		String routingKeys = "";

		for(String rQueue : routingKeyArr) {
			routingKeys += rQueue + "\n";
		}
		...
		String routeInfo = "routingKey : \n" + routingKeys + "routedQueue : \n" + routedQueues;
		...
		model.setRoutedQueues(routeInfo);
