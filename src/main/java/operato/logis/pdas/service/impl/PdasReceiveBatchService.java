 ...
 */
@Component
public class PdasReceiveBatchService extends AbstractQueryService {

	/**
	 * PDAS용 주문 수신 준비 서비스 (수신할 주문의 웨이브별 주문 건수 카운팅을 위한 커스텀 서비스)
	 */
	private static final String DIY_PDAS_READY_TO_RECEIVE_ORDER_SERVICE = "diy-pdas-ready-to-receive-order";
	/**
	 * PDAS용 웨이브별 주문 수신 처리 서비스
	 */
	private static final String DIY_PDAS_START_TO_RECEIVE_ORDER_SERVICE = "diy-pdas-start-to-receive-order";
	/**
	 * PDAS용 웨이브별 주문 수신 취소 처리 서비스
	 */
	private static final String DIY_PDAS_CANCEL_BATCH_SERVICE = "diy-pdas-cancel-batch";
	...
	/**
	 * PDAS 주문 정보 수신을 위한 수신 서머리 정보 조회
	 *  
	 * @param event
	 */
	@EventListener(classes = BatchReceiveEvent.class, condition = "#event.eventType == 10 and #event.eventStep == 1 and #event.jobType == 'PDAS'")
	 ...
	 * @param event
	 */
	@EventListener(classes = BatchReceiveEvent.class, condition = "#event.eventType == 20 and #event.eventStep == 1 and #event.jobType == 'PDAS'")
	public void handleStartToReceive(BatchReceiveEvent event) {
		...
		for(BatchReceiptItem item : items) {
			if(ValueUtil.isEqualIgnoreCase(LogisConstants.JOB_TYPE_PDAS, item.getJobType())) {
	 ...
	 * @param event
	 */
	@EventListener(classes = BatchReceiveEvent.class, condition = "#event.eventType == 30 and #event.eventStep == 1 and #event.jobType == 'PDAS'")
	public void handleCancelReceived(BatchReceiveEvent event) {
		...
		// 4. 커스텀 서비스 호출
		this.customService.doCustomService(domainId, DIY_PDAS_CANCEL_BATCH_SERVICE, ValueUtil.newMap("batch", batch));
	...
	private BatchReceipt readyToReceiveOrders(BatchReceipt receipt, String jobType, Object ... params) {
		...
		Object retObj = this.customService.doCustomService(receipt.getDomainId(), DIY_PDAS_READY_TO_RECEIVE_ORDER_SERVICE, ValueUtil.newMap("batchReceipt", receipt));
		
		if(retObj != null) {
			receiptItems = (List<BatchReceiptItem>) retObj;
		} else {
			throw ThrowUtil.newValidationErrorWithNoLog("Not found custom service [" + DIY_PDAS_READY_TO_RECEIVE_ORDER_SERVICE + "]");
		}
	...
	private BatchReceipt startToReceiveOrders(BatchReceipt receipt, BatchReceiptItem item, Object ... params) {
		...
		try {
			...
			this.customService.doCustomService(receipt.getDomainId(), DIY_PDAS_START_TO_RECEIVE_ORDER_SERVICE, customParams);
