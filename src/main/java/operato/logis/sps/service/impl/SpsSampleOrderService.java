 ...
 */
@Component
public class SpsSampleOrderService {
	/**
	 * 커스텀 서비스 - 대상 분류
	 */
	private static final String DIY_SPS_CREATE_ORDERS = "diy-sps-create-sample-orders";
	 ...
	 * @return 생성한 배치 ID
	 */
	@EventListener(classes = SampleOrderEvent.class, condition = "#event.jobType == 'SPS'")
	public void createSampleOrder(SampleOrderEvent event) {
		Object retVal = this.customService.doCustomService(event.getDomainId(), DIY_SPS_CREATE_ORDERS, ValueUtil.newMap("event", event));
	...
	private List<Order> createOrder(JobBatch batch, OrderSampler sampler, int orderIndex, int orderLineSeq, List<BoxType> boxTypeList, List<SKU> skuList) {
		...
		Order order = this.createOrderLine(batch, sampler, sku, invoiceId, boxTypeCd, orderIndex, orderLineIdx, sampler.getMaxOrderQty());
		orderList.add(order);
		orderLineIdx++;
		orderLineSeq++;
