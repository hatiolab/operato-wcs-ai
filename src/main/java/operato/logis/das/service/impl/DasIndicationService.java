...
public class DasIndicationService extends AbstractLogisService implements IDasIndicationService {
	...
	public void displayAllForCellCode(JobBatch batch) {
		String sql = "select c.ind_cd, c.cell_cd, g.gw_nm as gw_path from cells c inner join indicators i on c.domain_id = i.domain_id and c.ind_cd = i.ind_cd inner join gateways g on i.domain_id = g.domain_id and i.gw_cd = g.gw_cd where c.domain_id = :domainId and c.active_flag = :activeFlag and c.cell_cd in (select cell_cd from work_cells where domain_id = :domainId and batch_id = :batchId) order by c.cell_seq";
		Map<String, Object> params = ValueUtil.newMap("domainId,batchId,activeFlag", batch.getDomainId(), batch.getId(), true);
		List<IndCommonReq> indList = this.queryManager.selectListBySql(sql, params, IndCommonReq.class, 0, 0);
		
		IIndRequestService indReqSvc = this.getIndicatorRequestService(batch);
		for(IndCommonReq ind : indList) {
			indReqSvc.requestShowString(batch.getDomainId(), batch.getStageCd(), batch.getJobType(), ind.getGwPath(), ind.getIndCd(), ind.getIndCd(), ind.getCellCd());
		}
	}
