 ...
 * @author shortstop
 */
@Component("dpcBatchService")
public class DpcBatchService extends AbstractLogisService implements IBatchService {

	/**
	 * 커스텀 서비스 - 작업 완료 전 처리
	 */
	private static final String DIY_PRE_BATCH_STOP = "diy-dpc-pre-batch-stop";
	/**
	 * 커스텀 서비스 - 작업 완료 후 처리
	 */
	private static final String DIY_POST_BATCH_STOP = "diy-dpc-post-batch-stop";
	/**
	 * 커스텀 서비스
	 */
	@Autowired
	...
	protected void resetRacksAndCells(JobBatch batch) {
		...
		String sql = "UPDATE WORK_CELLS SET COM_Cd = null, CLASS_CD = null, BOX_ID = null, LAST_JOB_CD = null, LAST_PICKED_QTY = 0, JOB_INSTANCE_ID = null, STATUS = null WHERE DOMAIN_ID = :domainId AND BATCH_ID = :batchId";
	...
	protected void updateJobBatchFinished(JobBatch batch, Date finishedAt) {
		...
		String query = this.dpcBatchQueryStore.getBatchResultSummaryQuery();
		Map<String, Object> params = ValueUtil.newMap("domainId,batchId", batch.getDomainId(), batch.getId());
