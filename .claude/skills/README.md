...
- Gradle 빌드 실행 (`SKIP_FRONTEND=true ./gradlew clean build`)
- 프론트엔드 빌드 제외 (Docker 배포 시 Nginx에서 별도 서빙)
- 생성된 JAR 파일 검증 (파일 크기, 존재 여부)
- 빌드 결과 상세 보고

**사용법:**
```bash
# 기본 빌드 (테스트 스킵, 프론트엔드 제외)
/deploy-backend
...
- 백엔드만 빌드: `SKIP_FRONTEND=true ./gradlew build` (기본값)
...
**접속 정보:**
- 기본 URL: http://localhost:9190 (프로파일별 properties의 server.port)
...
3. `application.properties`의 server.port (기본: 9190)
4. Spring Boot 기본값 (9190)
...
**참고:**
- 백엔드 빌드는 기본적으로 `SKIP_FRONTEND=true` 옵션 사용
