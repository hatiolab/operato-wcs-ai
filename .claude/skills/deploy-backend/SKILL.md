...
2. **Java 버전 자동 전환**
   - Java 8 또는 11이 감지되면 Java 17로 자동 전환
   - 명령어: `export JAVA_HOME=$(/usr/libexec/java_home -v 17)`
   - Java 17이 없으면 사용 가능한 최신 버전 사용

### 2. 빌드 실행

**기본 빌드 명령어** (테스트 스킵, 프론트엔드 스킵):
```bash
export JAVA_HOME=$(/usr/libexec/java_home -v 17) && SKIP_FRONTEND=true ./gradlew clean build -x test
```

**옵션별 명령어**:
- `--with-tests`: 테스트 포함 빌드
  ```bash
  export JAVA_HOME=$(/usr/libexec/java_home -v 17) && SKIP_FRONTEND=true ./gradlew clean build
  ```

- `--no-clean`: clean 스킵 (증분 빌드)
  ```bash
  export JAVA_HOME=$(/usr/libexec/java_home -v 17) && SKIP_FRONTEND=true ./gradlew build -x test
  ```

**참고:**
- `SKIP_FRONTEND=true` 옵션은 프론트엔드 빌드를 건너뜁니다
- Docker 배포 시 프론트엔드는 별도로 빌드되므로 백엔드만 빌드합니다
- 프론트엔드 포함 전체 빌드: `./gradlew buildAll`
...
2. **프론트엔드 빌드 제외**
   - 기본적으로 `SKIP_FRONTEND=true` 옵션 사용하여 백엔드만 빌드
   - Docker 배포 시 프론트엔드는 Nginx에서 별도로 서빙
   - 프론트엔드 포함 빌드 필요 시: `./gradlew buildAll`

3. **빌드 시간**
   - 백엔드만 빌드: 약 8-15초 (테스트 스킵)
   - 프론트엔드 포함 전체 빌드: 약 1-2분 추가
