...
- **포트 결정 우선순위** (Spring Boot 표준):
  ...
  3. `application.properties`의 server.port (현재: 9190)
...
./gradlew bootRun --args='--spring.profiles.active=dev'
```
포트: `application-dev.properties`의 server.port (현재: 9190)
...
현재 사용 중인 프로세스:
java    12345 user   123u  IPv6 0x... TCP *:9190 (LISTEN)
