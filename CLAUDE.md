...
> 상세 기능 정의는 [`docs/requirements/ecs-module-feature-definition.md`](docs/requirements/ecs-module-feature-definition.md) 참조.
...
│   │   └── nginx.conf
│   └── Dockerfile
├── docs/                              # 프로젝트 산출물 문서 (상세 구조는 아래 참조)
│   ├── architecture/                  # 시스템 아키텍처 설계
│   ├── checklist/                     # 개발 체크리스트
│   ├── design/                        # API·DB·메시징 설계 명세
│   ├── implementation/                # 구현 가이드 (코딩 컨벤션 등)
│   ├── operations/                    # 운영·인프라·배포
│   ├── plans/                         # 개발 계획·WBS
│   ├── quality/                       # 코드 품질·보안·테스트 가이드
│   ├── refactoring/                   # 리팩토링 계획
│   ├── requirements/                  # 요구사항·기능 정의
│   ├── research/                      # 기술 리서치·트렌드 분석
│   └── sales/                         # 영업·가격 전략
├── docker-compose.yml
├── build.gradle
├── settings.gradle
...
> **유지 규칙:** `docs/` 하위에 폴더나 파일이 추가·변경·삭제되면, 위 구조를 함께 업데이트할 것.
...
| `hotfix/<이슈명>` | 긴급 운영 수정 | → `main`, `develop` |

- `main`에 직접 push 금지, PR을 통해 병합
- feature/fix 브랜치는 병합 후 삭제
...
**예시:**
- `VSCode 디버그 실행 설정 추가 — dev/factory 프로파일`
- `DB 연결 타임아웃 오류 수정 — 커넥션 풀 설정 조정`
- `DataSource 초기화 로직 리팩토링`
